...
class SelectionService:
    ...
    def save_selection(self, selected_ids: list[int]) -> list[InvoiceRecord]:
        """Persist selected records and return the selected records."""

        selected_count = self.repository.update_selection(selected_ids)
        logger.info("Selection saved | selected_count=%s ids=%s", selected_count, selected_ids)
        return self.repository.list_selected()

    def selected_records(self) -> list[InvoiceRecord]:
